package custom.custom_utilities;

public class AccountUtils {

    // SQLite Database User Table Column Names

    // Ids
    public static final String KEY_RECORD_ID = "Id";
    public static final String KEY_CLIENT_ID = "ClientId";

    // Names
    public static final String KEY_FIRST_NAME = "FirstName";
    public static final String KEY_LAST_NAME = "LastName";
    public static final String KEY_BUSINESS_NAME = "BusinessName";

    // Contacts
    public static final String KEY_PHONE_NUMBER = "PhoneNumber";
    public static final String KEY_EMAIL_ADDRESS = "EmailAddress";

    // Country
    public static final String KEY_COUNTRY_ITEM = "Country";
    public static final String KEY_COUNTRY_NAME = "CountryName";
    public static final String KEY_COUNTRY_CODE = "CountryCode";
    public static final String KEY_COUNTRY_ALPHA2 = "CountryAlpha2";
    public static final String KEY_COUNTRY_ALPHA3 = "CountryAlpha3";
    public static final String KEY_COUNTRY_FLAG = "CountryFlag";
    public static final String KEY_CITY_NAME = "CityName";

    // Password
    public static final String KEY_PASSWORD = "Password";
    public static final String KEY_CURRENT_PASSWORD = "CurrentPassword";
    public static final String KEY_NEW_PASSWORD = "NewPassword";
    public static final String KEY_CONFIRM_PASSWORD = "ConfirmPassword";

    // Gender
    public static final String KEY_GENDER = "Gender";
    public static final String KEY_GENDER_MALE = "Male";
    public static final String KEY_GENDER_FEMALE = "Female";
    public static final String KEY_GENDER_OTHER = "Other";

    // Account type
    public static final String KEY_ACCOUNT_TYPE = "AccountType";
    public static final String KEY_ACCOUNT_TYPE_PERSONAL = "AccountTypePersonal";
    public static final String KEY_ACCOUNT_TYPE_BUSINESS = "AccountTypeBusiness";

    // Email Verification
    public static final String KEY_EMAIL_VERIFIED = "EmailVerified";

    public static final String KEY_SIGNUP_DATE = "SignupDate";

    // User Profile
    public static final String KEY_UPDATE = "Update";
}
